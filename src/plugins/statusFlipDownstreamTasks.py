"""
When a Task status is flipped to 'cmpt' (Final), lookup each downstream Task that is currently
'wtg' (Waiting To Start) and see if all upstream Tasks are now 'cmpt'. If so, flip the downstream Task
...
def registerCallbacks(reg):
    ...
    reg.registerCallback('shotgunEventDaemon', 'e9f5a6858b086219257a432bc7e0a4b304d64ab4', flipDownstreamTasks, matchEvents, None)
...
def flipDownstreamTasks(sg, logger, event, args):
    ...
    for ds_task in sg.find("Task", ds_filters, fields):
        change_status = True
        # don't change status unless *all* upstream tasks are cmpt
